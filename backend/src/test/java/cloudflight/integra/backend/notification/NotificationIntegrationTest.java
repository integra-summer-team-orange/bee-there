...
public class NotificationIntegrationTest {
    ...
    void shouldProveFullRoundTrip() throws Exception {
        ...
        mockMvc.perform(put("/api/notification/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id))
                .andExpect(jsonPath("$.type").value(NotificationType.WEATHER_ALERT))
                .andExpect(jsonPath("$.name").value("updated"));

        // 4. DELETE
        mockMvc.perform(delete("/api/notification/{id}", id)).andExpect(status().isNoContent());

        // 5. VERIFY DELETE
        mockMvc.perform(get("/api/notification/{id}", id)).andExpect(status().isNotFound());
    }
    ...
    void shouldGetAllNotificationsPaged() throws Exception {
        ...
        mockMvc.perform(get("/api/notification?pageNumber=0&pageSize=10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(greaterThanOrEqualTo(2))))
                .andExpect(jsonPath("$.content[*].name", hasItems("Notification 1", "Notification 2")));
    }
}
