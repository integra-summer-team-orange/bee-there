...
public class CoffeeController {
    ...
    @ApiResponse(responseCode = "200", description = "Successfully retrieved list of coffees",
        content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = CoffeeDto.class))))
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved the coffee",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = CoffeeDto.class))),
        @ApiResponse(responseCode = "404", description = "Coffee not found", content = @Content)
    })
    public CoffeeDto getById(
        @Parameter(description = "ID of the coffee to be retrieved", required = true)
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Coffee successfully created",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = CoffeeDto.class))),
        @ApiResponse(responseCode = "400", description = "Invalid input data", content = @Content)
    })
    public ResponseEntity<CoffeeDto> create(
        @Parameter(description = "Coffee object to create", required = true)
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Coffee successfully updated",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = CoffeeDto.class))),
        @ApiResponse(responseCode = "404", description = "Coffee not found", content = @Content),
        @ApiResponse(responseCode = "400", description = "Invalid input data", content = @Content)
    })
    public CoffeeDto update(
        @Parameter(description = "ID of the coffee to be updated", required = true)
        @PathVariable Long id,
        @Parameter(description = "Updated coffee object data", required = true)
        @RequestBody CoffeeDto dto) {
    ...
    public void delete(
        @Parameter(description = "ID of the coffee to be deleted", required = true)
        @PathVariable Long id) {
        service.delete(id);
    }
}
